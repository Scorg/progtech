			...
			try {
				using (SpreadsheetDocument doc = SpreadsheetDocument.Create(filename, SpreadsheetDocumentType.Workbook)) {
					...
					using (var ctx=new MongoContext()) {
						...
						Dictionary<int, string> statuses = ctx.Set<OrderStatus>().AsQueryable().ToDictionary(x => x.id, x => x.name);
						Dictionary<int, Product> products = new Dictionary<int, Product>();
						
						IQueryable<Order> query = ctx.Set<Order>().AsQueryable().OrderByDescending(x => x.date).Skip(skip);
						if (take>=0) query = query.Take(take);
						...
						foreach (Order o in list) {
							...
							if (bRelated) {
								foreach (OrderProduct op in o.products) {
									...
									if (!products.TryGetValue(op.product_id, out p)) {
										p = ctx.Set<Product>().Find(x => x.id == op.product_id).FirstOrDefault();

										if (p==null) p = new Product { name = "(не найден)" };
										products.Add(op.product_id, p);
									}
									...
									r.Append(
										...
										new Cell { DataType = CellValues.String, StyleIndex = 0, CellValue = new CellValue(p.name) },
