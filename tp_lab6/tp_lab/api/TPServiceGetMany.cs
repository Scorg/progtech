	...
	partial class TPService : ITPService {
		public List<types.Attribute> GetAttributes(int skip, int take)
		{
			using (Entities ctx=new Entities()) {
				try {
					return ctx.attribute.Select(x => new types.Attribute { id=x.id, Name=x.name }).OrderBy(x => x.Name).Skip(skip).Take(take).ToList();
				} catch (Exception ex) {
		...
		public List<types.Category> GetCategories(int skip, int take)
		{
			using (Entities ctx=new Entities()) {
				try {
					return ctx.category.Select(x => new types.Category { id=x.id, ParentID=x.parent_id, Name=x.name }).OrderBy(x => x.Name).Skip(skip).Take(take).ToList();
				} catch (Exception ex) {
					common.LogFile.Log(ex);
				}
